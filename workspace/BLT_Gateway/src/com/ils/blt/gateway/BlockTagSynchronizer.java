...
public class BlockTagSynchronizer {
	...
	public BlockTagSynchronizer(String proj) {
		this.handler = ControllerRequestHandler.getInstance();
		this.projectName = proj;
		this.isolationProvider = handler.getProjectIsolationTagProvider(projectName);
		this.productionProvider= handler.getProjectProductionTagProvider(projectName);
	}
	...
	public void synchBlocks(ProcessDiagram diagram) {
		...
		for(ProcessBlock block:diagram.getProcessBlocks()) {
			...
			if( block.getClassName().equals(BlockConstants.BLOCK_CLASS_SOURCE)) {
				...
				if(prop != null ) {
					List<SerializableBlockStateDescriptor> sinks = handler.listSinksForSource(diagram.getResourceId(), block.getBlockId().toString());
					if(sinks.size()>0) {
	...
	public void synchDeletedBlock(ProcessBlock block) {
		if( block.getClassName().equals(BlockConstants.BLOCK_CLASS_SINK)) {
			...
			handler.deleteTag(projectName,TagUtility.replaceProviderInPath(isolationProvider,path));
			handler.deleteTag(projectName,TagUtility.replaceProviderInPath(productionProvider,path));
		}
	}
