...
public class TagListener implements TagChangeListener   {
	 ...
	 */
	public void defineSubscription(CoreBlock block,BlockProperty property,String tagPath) {
	 ...
	 * @param tagPath
	 */
	public void removeSubscription(CoreBlock block,BlockProperty property,String tagPath) {
	...
	private void startSubscriptionForTag(String tagPath) {
		...
		CoreBlock typicalBlock = list.get(0).getBlock();
		try {
		...
		if( tag!=null && tag.getValue()!=null && tp!=null ) {
			try {
				...
				for(BlockPropertyPair key:list) {
					...
					BasicDiagram parent = controller.getDiagram(block.getParentId());
					if( parent!=null ) {
	...
	private void updateProperty(CoreBlock block,BlockProperty property,QualifiedValue value) {
		try {
	...
	// ====================================== ProjectResourceKey =================================
		/**
		 * Class for keyed storage by CoreBlock and Property
		 */
		private class BlockPropertyPair {
			private final CoreBlock block;
			private final BlockProperty property;
			public BlockPropertyPair(CoreBlock blk,BlockProperty prop) {
				this.block = blk;
				this.property = prop;
			}
